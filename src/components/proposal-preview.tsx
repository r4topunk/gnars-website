...
export function ProposalPreview({ data }: ProposalPreviewProps) {
  ...
  const getTransactionIcon = (type: string) => {
    switch (type) {
      case 'send-eth': return Send
      case 'send-tokens': return Coins
      case 'send-nfts': return ImageIcon
      case 'droposal': return Zap
  ...
  const getTransactionLabel = (type: string) => {
    switch (type) {
      case 'send-eth': return 'Send ETH'
      case 'send-tokens': return 'Send Tokens'
      case 'send-nfts': return 'Send NFTs'
      case 'droposal': return 'Create Droposal'
  ...
  const canSubmit = data.title && data.transactions.length > 0 && !isSubmitting && !isPending && !isConfirming
