...
class InfiniteGridMenu {
  ...
  private IDLE_TIMEOUT = 2000; // Pause render after 2s of no interaction
  private animationFrameId: number | null = null;
  public cleanup: (() => void) | null = null;

  private loadedImageCount = 0;
  private INITIAL_LOAD_COUNT = 20; // Load first 20 images immediately
  private BATCH_SIZE = 15; // Then load 15 at a time
  ...
  public run(time = 0): void {
    ...
    // Only continue animation loop if not idle
    if (!this.isIdle) {
  ...
  private resumeRendering(): void {
    if (this.animationFrameId === null) {
      this.isIdle = false;
      // Reset time to prevent large deltaTime jump after tab was inactive
  ...
  private init(onInit?: InitCallback): void {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        // Tab became visible - set flag to resume on next user interaction
  ...
  private drawAvatarOverlay(ctx: CanvasRenderingContext2D, avatar: HTMLImageElement, x: number, y: number, cellSize: number): void {
    ...
    ctx.lineWidth = Math.max(4, cellSize * 0.02); // Thicker border
    ctx.beginPath();
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (isMoving !== this.movementActive) {
      ...
      if (isMoving) {
        ...
        // Schedule pause when movement stops
...
const ReactBitsInfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], onActiveItemChange }) => {
  ...
  useEffect(() => {
    ...
    const handleMovementChange = () => {
      // Movement state handled by parent if needed
    };
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      {/* Built-in UI elements hidden - replaced by parent component */}
    </div>
  );
};
