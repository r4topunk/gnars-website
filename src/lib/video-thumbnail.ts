...
export const generateVideoThumbnail = async (videoFile: File): Promise<File> => {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          "Video thumbnail generation timed out. The video format may not be supported for thumbnail extraction.",
        ),
      );
    ...
    video.onloadedmetadata = () => {
      try {
        ...
        reject(
          new Error(
            `Failed to process video metadata: ${error instanceof Error ? error.message : "Unknown error"}`,
          ),
        );
      }
    };
    ...
    video.onseeked = () => {
      try {
        ...
        reject(
          new Error(
            `Failed to draw video frame: ${error instanceof Error ? error.message : "Unknown error"}`,
          ),
        );
      }
    };
    ...
    try {
      ...
      reject(
        new Error(
          `Failed to load video file: ${error instanceof Error ? error.message : "Unknown error"}`,
        ),
      );
    }
  });
